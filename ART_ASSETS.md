...
角色会跨图出现，**必须按顺序生成，用前一步的定稿当参考图**，否则同一人
在不同图里会长得不一样：

1. **先出 B1–B5 五人全身立绘**：每人多抽几张挑定稿（发色/瞳色/服装
   以 prompt 为准），定稿即该角色的唯一标准像
2. **V 系列头像直接从定稿立绘裁切/局部重绘**，不要重新生成
3. **C（状态插画）、H（隐藏面）、A2（合影）里出现的角色**，一律把
   对应定稿立绘喂进参考图通道（img2img / character reference /
   IP-Adapter，看用哪家工具）再生成
4. 全程**同一个模型 + 同一套风格参数**，中途别换模型——通用风格块
   只能保证气质，跨模型必然跑脸

## 构图与裁切安全区

- **横幅插画（C1–C5）**：主体居中，四边各留约 10% 裁切安全区——
  页面会按宽度裁切，关键元素（脸、手、打翻的杯子）别贴边
- **横幅边缘处理**：衬在奶油底 `#F6F1E7` 上显示，画面边缘向奶油色
  过渡（或直接交透明底 PNG），避免生硬的矩形边框感
- **A2 官网主视觉**：左三分之一留干净背景给标题文案，五人集中在
  中右侧；上下各留 10%（响应式会裁上下）
- **立绘（B/H 系列）**：人物顶天立地但头顶与脚下各留 5%，
  透明底，别画地面阴影（页面自己加）
- **头像（V/G 系列）**：胸像构图，脸部占画面约 50%，居中，
  会被裁成圆形显示——四角不要放重要元素
